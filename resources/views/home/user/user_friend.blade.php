                                ...
                                <li data-id="111" class="jieda-daan" style="margin-bottom:5px; padding-top: 10px; border-bottom: 1px solid #DFDFDF;">
                                    <div class="detail-about detail-about-reply" style="padding-left:40px; margin-bottom: -10px">
                                        <div class="fly-detail-user" style="font-size: smaller">
                                            <img class="layui-circle fly-avatar" src="{{ $friend->avatar }}" alt=" " style="width: 35px;height: 35px">
                                            <a href="{{ url('authorInfo?id=').$friend->id }}" class="fly-link" style="font-size: small">
                                                <cite style="font-style: italic">{{ $friend->username }}</cite>
                                            </a>
                                        </div>
                                        <span style="font-size: smaller">{{ $friend->city }}</span>
                                        <span style="font-size: smaller">{{ $friend->profession }}</span>
                                        <span style="font-size: smaller">recently</span>
                                        <div class="jieda-admin">
                                            <a href="javascript:void(0)" >
                                                <span class="" >
                                                    <em class="" onclick="sendPrivateMessage({{ $friend->id }})" style="font-size: smaller">发送私信</em>
                                                </span>
                                            </a>
                                            &nbsp;&nbsp;
                                            <a href="javascript:void(0)" onclick="cancelFriend({{ $friend->id }})">
                                                <span class="" >
                                                    <em class="" style="font-size: smaller">删除好友</em>
                                                </span>
                                            </a>
                                        </div>
            ...
            layui.use('laypage', function(){
                ...
                laypage.render({
                    ...
                    jump: function(obj, first){
                        ...
                        if(!first){
                            //do something
                            window.location.href = '{{ url('/user/userFriend?page=') }}'+obj.curr;
                        }
                    }
...
<script>
    ...
    {
        layer.confirm('确定要删除好友吗', {skin: 'layui-layer-lan',},function(){
            var friend_id = id;
            $.ajax({
                url: "{{ url('/user/cancelFriend') }}",
                dataType: 'json',
                type: 'GET',
                data: {'id': friend_id},
                success: function(e) {
